...
def mock_distance_repo() -> AsyncMock:
    repo = AsyncMock()
    repo.upsert_by_pair.return_value = (None, True)
    return repo


@pytest.fixture
def location_handler(mock_location_repo: AsyncMock) -> LocationImportHandler:
    return LocationImportHandler(repository=mock_location_repo)


@pytest.fixture
def distance_handler(
    mock_distance_repo: AsyncMock, mock_location_repo: AsyncMock,
...
class TestLocationImportHandler:
    ...
    async def test_handle_upserts_locations(
        self, location_handler: LocationImportHandler, mock_location_repo: AsyncMock,
    ) -> None:
        event = EventDocument(
...
class TestDistanceImportHandler:
    ...
    async def test_handle_skips_unchanged(
        ...
        mock_distance_repo.upsert_by_pair.return_value = (None, False)
